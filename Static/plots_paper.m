...
load PaperEx1.mat
figure;
h1 = plot(lead(1,:),J(:,1),'-rs','LineWidth',1.2,'MarkerSize',6); hold on;
h2 = plot(lead(1,:),J(:,3),'-g+','LineWidth',1.2,'MarkerSize',6); hold on;
h3 = plot(lead(1,:),J(:,5),'-bo','LineWidth',1.2,'MarkerSize',6); hold on;
h4 = plot(lead(1,:),J(:,7),'-kv','LineWidth',1.2,'MarkerSize',6);
...
% Set legend
h = legend([h1,h2,h3,h4],'block-diagonal','sparsity invirance with $T = S$', 'sparsity invirance with $T = T_{new}$','centralized');
