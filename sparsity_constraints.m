...
% tic
% [indi,indj] = find(Tbin == 0);   % the position with zero entries
% for i = 1:length(indi)
%     Constraints = [Constraints, CQv(indi(i),(indj(i)-1)*N+1:indj(i)*N) == 0, DQv(indi(i),indj(i)) == 0];
% end
% toc
        ...
        for j = 1:n
            ...
            if Rbin(i,j) == 0                                                                % Whenever we need GY(i,j)=0....
                ...
                for l = 1:m
                    ...
                    Qslj = CQs(l,(j-1)*N+1:j*N) * Gi + DQs(l,j);
