...
eq1 = inv(eye(n)-Gs*Yr*inv(eye(n)+Gs*Yr)); %%(I-G*K)^-1=(I-G*Y*X^-1)^-1=(I-G*Y*(I+G*Y)^-1)^-1
for i = 1:n
    for j = 1:n
        fprintf('   Percentage %6.4f \n', 100*(n*(i-1)+j)/n/n )
        
        if(isstable(syms2tf(eq1(i,j)))==0)
...
eq2 = Yr*inv(eye(n)+Gs*Yr)*inv(eye(n)-Gs*Yr*inv(eye(n)+Gs*Yr));
for i = 1:m
    for j = 1:n
        fprintf('   Percentage %6.4f \n', 100*(n*(i-1)+j)/n/n )
        
        if(isstable(syms2tf(eq2(i,j)))==0)
            fprintf('   Apparently unstable: check close zero/pole cancellations\n')
...
eq3 = inv(eye(n)-Gs*Yr*inv(eye(n)+Gs*Yr))*Gs;
for i = 1:n
    for j = 1:m
        fprintf('   Percentage %6.4f \n', 100*(n*(i-1)+j)/n/n )
...
eq4 = inv(eye(n)-Yr*inv(eye(n)+Gs*Yr)*Gs);
for i = 1:m
    for j = 1:m
        fprintf('   Percentage %6.4f \n', 100*(n*(i-1)+j)/n/n )
        if(isstable(syms2tf(eq4(i,j)))==0)
            fprintf('   Apparently unstable: check close zero/pole cancellations\n')
